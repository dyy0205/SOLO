    ...
    for V in progressbar:
        ...
        if isinstance(Fs, list):
            ...
            for idx, f in enumerate(Fs):
                if idx == 1:
                    seg_results = mask_inference(video_name, target_shape, SOLO_INTERVAL, SCORE_THR, hflip=True, with_refine=WITH_REFINE)
                elif idx == 2:
                    seg_results = mask_inference(video_name, TEST_SCALE, SOLO_INTERVAL, SCORE_THR, hflip=False, with_refine=WITH_REFINE)
                else:
                    seg_results = mask_inference(video_name, target_shape, SOLO_INTERVAL, SCORE_THR, hflip=False, with_refine=WITH_REFINE)
            ...
            seg_results = mask_inference(video_name, target_shape, SOLO_INTERVAL, SCORE_THR, with_refine=WITH_REFINE)
...
@fn_timer
def mask_inference(video_name, mask_shape, interval, score_thr, hflip=False, with_refine=False):
    ...
    for f in fi:
        ...
        if result is None:
            continue
        if with_refine:
            result = process_solo_result_with_refine(refine_model, img, result, mask_shape)
        else:
...
def process_solo_result_with_refine(refine_net, img, result, mask_shape):
    masks = result[0].cpu().numpy().astype(np.uint8)
    cates = result[1].cpu().numpy()
    scores = result[2].cpu().numpy()
    h0, w0 = masks[0].shape
    ...
    for i, mask in enumerate(masks):
        ...
        img_x = cv2.resize(img_crop, (384, 384), cv2.INTER_LINEAR)
        mask_x = cv2.resize(mask_crop, (384, 384), cv2.INTER_NEAREST)
    ...
    final = []
    for i in range(num):
        mask = cv2.resize(masks[i], mask_shape, cv2.INTER_NEAREST)
        result_.append(mask)
    final.append(np.array(result_))
    final.append(cates)
    final.append(scores)
...
if __name__ == '__main__':
    ...
    if MODE == 'online':
        ...
        CONFIG_FILE = r'/home/dingyangyang/SOLO/cfg/aug_solov2_r101.py'
        CKPT_FILE = r'/home/dingyangyang/SOLO/solov2_9cls.pth'
    ...
    refine_ckpt_path = '/home/dingyangyang/SOLO/refine_044.ckpt'
