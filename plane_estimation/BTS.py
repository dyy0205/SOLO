...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal, device):
...
class bts(nn.Module):
    ...
    def forward(self, features, focal, device):
        ...
        daspp_feat = self.daspp_conv(concat4_daspp)  # H/8, 128
        ...
        depth_8x8 = self.lpg8x8(plane_eq_8x8, focal, device)
        ...
        depth_4x4 = self.lpg4x4(plane_eq_4x4, focal, device)
        ...
        depth_2x2 = self.lpg2x2(plane_eq_2x2, focal, device)
...
class BtsModel(nn.Module):
    ...
    def forward(self, x, focal=512, device='cuda:0'):
        skip_feat = self.encoder(x)
        return self.decoder(skip_feat, focal, device)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i, img in enumerate(imgs):
            ...
            Image.fromarray(depth).convert('I').resize((raw_w, raw_h)).save(img.replace('.jpg', '.png'))

            # points = convert_3dpoints(depth, 480, 640)
            # np.savez(os.path.join(args.save_dir, name.replace('.jpg', '.npz')),
            #          bts_depth=depth, points=points, last_feat=feat)
