...
class SOLOHead(nn.Module):
    ...
    def _init_layers(self):
        ...
        # attention module
        channel_module = [
            nn.Conv2d(self.seg_feat_channels, int(self.seg_feat_channels * 0.5), 1),
            nn.Conv2d(int(self.seg_feat_channels * 0.5), self.seg_feat_channels, 1)
        ]
        self.channel_attention = nn.Sequential(*channel_module)
        self.spatial_attention = nn.Conv2d(2, 1, 3, padding=1)
    ...
    def forward_single(self, x, idx, eval=False, upsampled_size=None):
        ...
        if self.with_attention:
            N, c, h, w = ins_feat.shape

            # channel attention
            avg_pool = ins_feat.mean([2, 3], keepdim=True)  # [N, c, 1, 1]
            max_pool = ins_feat.view(N, c, -1).max(2, keepdim=True)[0].unsqueeze(-1)
            avg_pool = self.channel_attention(avg_pool)
            max_pool = self.channel_attention(max_pool)
            ins_feat = ins_feat.mul((avg_pool + max_pool).sigmoid())

            # spatial attention
            avg_pool = ins_feat.mean(1, keepdim=True)  # [N, 1, h, w]
            max_pool = ins_feat.max(1, keepdim=True)[0]
            pool = torch.cat([avg_pool, max_pool], dim=1)
            ins_feat = ins_feat.mul(self.spatial_attention(pool).sigmoid())
        ...
        if eval:
            ins_pred = F.interpolate(ins_pred.sigmoid(), size=upsampled_size, mode='bilinear', align_corners=True)
